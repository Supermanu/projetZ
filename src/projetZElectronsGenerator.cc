/*
    ProjetZ est un programme de simulation de la désintégration d'un boson de
    Higgs de 200 Gev
...
projetZElectronsGenerator::projetZElectronsGenerator() : masseHiggs ( 200.0*GeV
                                                                        ), directionHiggs ( 0,0,1 )
{
    srand ( time ( NULL ) );
    // on va d'abord calculer l'énergie des Z pour un H au repos, ensuite
    // l'énergie pour des électrons avec un Z au repos.
...
//   G4double EnergieCinetiqueH = 0.;
    G4double vitesseH = sqrt ( 1 - ( 1/ ( ( ( EnergieCinetiqueH/masseHiggs ) +1
                                          ) * ( ( EnergieCinetiqueH/masseHiggs ) +1 ) ) ) );
    G4cout << "Energie cinétique du H: " << G4BestUnit ( EnergieCinetiqueH,
            "Energy" ) << endl;
    G4cout << "Vitesse du H: " << vitesseH << endl;
    G4double pElectron = sqrt ( ( 91.187*GeV*91.187*GeV ) /4 -
                                0.511*MeV*0.511*MeV );
    ...
    // On va maintenant distribuer la quantité de mouvement pour un angle
    // aléatoire
    ...
    pVectorElectron[1] = pElectron * cos ( angleAleatoire[0] ) *sin (
                             angleAleatoire[1] );
    pVectorElectron[2] = pElectron * sin ( angleAleatoire[0] ) *sin (
                             angleAleatoire[1] );
...
vector<G4double> projetZElectronsGenerator::boost ( vector<G4double>
        p_i,vector<G4double> b )
{
...
{
    G4double pElec = sqrt (
                         momentumElectrons[electron][1]*momentumElectrons[electron][1] +
                         momentumElectrons[electron][2]*momentumElectrons[electron][2] +
                         momentumElectrons[electron][3]*momentumElectrons[electron][3] );
    G4ThreeVector moment (
        momentumElectrons[electron][1]/pElec,momentumElectrons[electron][2]/pElec,
        momentumElectrons[electron][3]/pElec );
    return moment;
}
